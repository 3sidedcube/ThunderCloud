...
class GunzipTests: XCTestCase {
    ...
    func testGunzip() throws {
        ...
        guard let url = Bundle.module.url(forResource: "test", withExtension: "tar.gz") else {
            XCTFail("test.tar.gz missing from test bundle")
            return
        }
        
        guard let responseUrl = Bundle.module.url(forResource: "test_ungzipped_base64", withExtension: "txt") else {
