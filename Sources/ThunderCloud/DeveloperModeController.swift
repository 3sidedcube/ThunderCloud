...
public class DeveloperModeController: NSObject {
    ...
    public func loginToDeveloperMode() {
        ...
        if !DeveloperModeController.appIsInDevMode {
            ...
            guard let loginViewController = UIStoryboard(name: "Login", bundle: .module).instantiateInitialViewController() as? StormLoginViewController else { return }
            
            loginViewController.loginReason = "Log in using your Storm login to enter Dev Mode"
            
            let storyboard = UIStoryboard(name: "DeveloperMode", bundle: .module)
